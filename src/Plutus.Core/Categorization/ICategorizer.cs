...
/// <summary>The category the model selected, a suggested plain-English note, and its confidence (0–1).</summary>
public sealed record CategorizationResult(string Category, string? Note, double Confidence);

public interface ICategorizer
{
    /// <summary>
    /// Asks the configured model to pick the best-fit category for a transaction, constrained to the
